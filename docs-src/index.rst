...
`/people/find <https://docs.everyaction.com/reference/people-find>`__ to find a person with the phone
...
out the :doc:`services documentation <services>` to see a list of the services as well as what their methods are,
and to which HTTP endpoint they correspond to. Some methods, such as :meth:`.People.lookup`, are implemented for user
...
in a way which is more convenient than using a more direct mechanism such as :code:`cURL` or directly using
...
`GET /people/{vanId} <https://docs.everyaction.com/reference/get-people-vanid>`__ using :meth:`.People.get`
...
Note that the `alias <Aliases>`_ :code:`view_restricted` is used instead of the original data parameter name
:code:`isViewRestricted` in the previous example.
...
`expands <https://docs.everyaction.com/reference/expansion>`__ their phones and emails (note that
...
As a final note, :code:`$expand` is handled as a special case, allowing using to specify a sequence of strings if they
...
The following code creates an :class:`.Event` with an :class:`EventType` with ID :code:`5555` and
:class:`voter registration batches <.VoterRegistrationBatch>` that have the IDs :code:`141`, :code:`173`, and
:code:`200` respectively:
...
names which appear in the documentation. In particular, when specifying an object with an ID, the alias :code:`id` may
be used instead of the full property name. Additionally, :class:`.Event` objects recognize the aliases :code:`type`
for :code:`eventType` and :code:`batches` for :code:`voterRegistrationBatches`. Thus, the previous code may be made
much more compact:
...
* `EveryAction Objects`_ which have IDs may always be constructed by specifying the ID as the first parameter. For
  example, :code:`event_type = EventType(5555)` constructs an :class:`.EventType` with the ID :code:`5555`.
...
In additional to usual aliases, there is also a special kind of alias called a "singular alias" which is an alias
that may be used to conveniently specify a single value where multiple values are expected. For example, if we only
needed to specify one voter registration batch with the ID 141, we could use the singular alias :code:`batch` for
:code:`voterRegistrationBatches` to do the following:
...
* Properties may have multiple aliases, or no aliases at all (like for the property :code:`name`).
* The snake_cased version of a property name is *always* an alias for that property (i.e., :code:`is_view_restricted`
  is an alias for :code:`isViewRestricted`).
...
* When an object has a property which is an ID for another kind of object, the property name without "Id" is almost
  always an alias for that property, such as :code:`designation` for :code:`designationId` in
...
`Common Models section for People <https://docs.everyaction.com/reference/common-models>`__, the various
...
such as an :class:`.ActivistCode` for the :meth:`.ActivistCodes.get` method. Additionally, when such objects have other
EveryAction objects as properties, the value of those properties will also be an instance of the corresponding Python
class. Like endpoint methods, these classes make use of `Aliases`_ to allow for more succinct property access:
for instance, the :class:`.Person` property :code:`lastName` can be accessed with the aliases :code:`last_name` and
:code:`last`:
...
Unpacking :code:`code` with :code:`**` has the same affect as using keyword arguments with it's corresponding properties
...
ID or a name, the corresponding EAObject takes an optional positional parameter in its constructor which will set the
ID (if it is an :code:`int`) or the name (if it is a :code:`str`) of the resulting object. This is the reason why,
for the previous example, the :class:`SupportedEntities <.SupportedEntity>` accepted positional arguments, as well
as the created :class:`.Code` object: in both cases, this has the same effect as using :code:`name='...'`.

Paginated Methods
-----------------

Paginated methods, such as :meth:`.ActivistCodes.list`, support the :code:`limit` and :code:`skip` keyword arguments:
...
Additionally, setting limit to 0 makes the number of results unlimited. If :code:`default_limit` is set to 0, then an
unlimited amount of results will be retrieved when :code:`limit` is unspecified.
